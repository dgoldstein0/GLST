...
{
	private static final long serialVersionUID = -2734123234873525796L;
	...
	public GalacticMapPainter(boolean game_mode)
	{
		...
		max_dist_shown=Constants.DEFAULT_DIST;
		nav_level=Constants.DEFAULT_NAV_LEVEL;
		scale=1.0d;
	...
	public void paintComponent(Graphics g)
	{
		...
		if(map != null && map.getSystems() != null)
		{
					...
					else if(disp_names)
					{
						...
						if(name != null) {
							if(sys.navigability < nav_level)
								...
								g2.setColor(sys.currentColor(time));
							g2.drawString(name, (float)scaleNum(sys.x+3), (float)scaleNum(sys.y)+m.getHeight());
						} else {
			...
			{
				g2.setColor(GameInterface.GC.getPlayers()[GameInterface.GC.getPlayer_id()].getColor());
				for(Ship s : ships_in_transit)
				{
					...
					//draw ship s
					g2.rotate(s.getExit_direction()-Math.PI/2, scale_x, scale_y);
					g2.draw(triangle);
		...
		g2.setColor(Color.WHITE);
		g2.draw(new Rectangle2D.Double(0,0,scaleNum(Constants.GALAXY_WIDTH), scaleNum(Constants.GALAXY_HEIGHT)));
	}
