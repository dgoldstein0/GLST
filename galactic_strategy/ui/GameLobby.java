...
{
	private static final long serialVersionUID = -2712712665702478377L;
	...
	public GameLobby(JFrame f, GameControl gc)
	{
		...
		is_hosting = GC.getPlayers()[GC.getPlayer_id()].isHosting();
		...
		JLabel[] player_nums = new JLabel[players.length];
		player_names = new JLabel[players.length];
		color_samples =  new JPanel[players.length];
		color_nums = new Integer[players.length];
		...
		{	
			...
			if(players[i] != null){
				player_names[i].setText(players[i].getName());
				if(players[i].getReady())
					player_names[i].setForeground(Color.GREEN);
				else
					player_names[i].setForeground(Color.RED);
				color_samples[i].setBackground(players[i].getColor()); //player.getColor() returns the default color here, since color is not yet set
				color_nums[i] = i;
			}
			...
			if(!map_label.getText().equals(LOAD_MAP_MSG)){
				...
				for(int i=0; i< players.length; i++){
					if(color_nums[i] != null){ 
						...
						if(i != 0 && is_hosting && !players[i].getReady())//color_nums[i] instanceof Integer iff players[i] instanceof Player... else error here...
	...
	public void mouseClicked(MouseEvent e){
		if (!GC.getPlayers()[GC.getPlayer_id()].getReady()){ //once ready is clicked by a client, the colors cannot be altered
			for (int i=0; i<color_samples.length; i++){
				if (e.getSource() == color_samples[i]){
					if (GC.getPlayers()[i] != null){
						if(e.getButton() == MouseEvent.BUTTON1){
							color_nums[i]++;
							if(color_nums[i] >= Constants.DEFAULT_COLORS.length)
								color_nums[i]=0;
						}
						else if(e.getButton() == MouseEvent.BUTTON3){
							color_nums[i]--;
							if(color_nums[i] <0)
								color_nums[i] = Constants.DEFAULT_COLORS.length-1;
						}
						GC.getPlayers()[i].setColor(Constants.DEFAULT_COLORS[color_nums[i]]);
						color_samples[i].setBackground(GC.getPlayers()[i].getColor());
					}
				}
			}
	...
	public void maybeEnableStartGame() {
		//check to see if the player was the only thing holding up the game - or
		//allowing it to start.  If so, enable/disable start button appropriately
		start_game.setEnabled(readyToStart());
	}
}