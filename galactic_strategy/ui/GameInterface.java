...
import galactic_strategy.ui.FormationCommandPanel;
import galactic_strategy.user_actions.ShipAttackMoveOrder;
import galactic_strategy.user_actions.ShipAttackOrder;
import galactic_strategy.user_actions.ShipMoveOrder;
import galactic_strategy.user_actions.ShipWarpOrder;
...
public class GameInterface implements MouseListener, WindowListener, ComponentListener, MouseWheelListener, AWTEventListener
{
	...
	public static enum GALAXY_STATE{NORMAL, CHOOSE_WARP_DEST, PREVIEW;}
	...
	public SystemCommandPanel SysPanel;
	FormationCommandPanel FormationPanel;
	...
	public void drawGalaxy(GALAXY_STATE state)
	{
		...
		gal_scale =  Math.min(((double)theinterface.getWidth())/((double)Constants.GALAXY_WIDTH), ((double)theinterface.getHeight())/((double)Constants.GALAXY_HEIGHT));
		GalaxyPanel.paintGalaxy(
				GC.map,
				selected_sys,
				(galaxy_state == GALAXY_STATE.CHOOSE_WARP_DEST) ? GDFrame.DRAG_RANGE : GDFrame.DRAG_NONE,
				Constants.MAX_NAV_LEVEL,
				GDFrame.NAV_DISP_NONE,
				false,
				GC.getPlayers()[GC.getPlayer_id()].getShips_in_transit(),
				gal_scale);
...
/*		sys_scale = 1.0d;
	...
	private void doMouseDragged(MouseEvent e)
	{
		if (sys.getOwner_id() == GC.getPlayer_id() || sys.getOwner_id() == -1 || !Constants.fogofwar)
		{
			...
			for(Iterator<Selectable> select_it = overall_view_ships.iterator();select_it.hasNext();)
			{
				...
				if(!(obj instanceof Ship) || ((Ship)obj).getOwner().getId() != GC.getPlayer_id())
				{
					select_it.remove();
				}
			}
							...
							for(Selectable s : selected_in_sys)
							{
								if(s != null) //TODO: why is it null?
								{
									...
									if(x_dif*x_dif + y_dif*y_dif <= range*range)
									{
										...
										some_warped=true;
										GC.scheduleOrder(
											new ShipWarpOrder(GC.getPlayers()[GC.getPlayer_id()],
															  (Ship)s,
	 ...
	 * 
	 * @see Constants.SELECTION_TOLERANCE
	 * */
	private GSystem findSystemAt(double x, double y)
	{
		for(GSystem the_sys : GC.map.getSystems()) //TODO: this should search through GC.players[GC.player_id].known_systems
		{
			double dif_x = the_sys.x - x;
			double dif_y = the_sys.y - y;
			
			if(dif_x*dif_x + dif_y*dif_y <= Constants.SELECTION_TOLERANCE*Constants.SELECTION_TOLERANCE)
				return the_sys;
		}
	...
	{
		Set<Star> stars = sys.getStars();
		if(stars != null)
		{
			for(Star st : stars)
			{
				//search for star...
				if(st.getX()-st.getSize()/2 <= x2 && x1 <= st.getX()+st.getSize()/2 &&
					st.getY()-st.getSize()/2 <= y2 && y1 <= st.getY()+st.getSize()/2)
				{
					select_items.add(st);
				}
			}
		...
		if(sys.getOrbiting() != null)
		{
			for(Satellite<?> orbiting : sys.getOrbiting())
			{
				//search for satellites...
				if(orbiting.absoluteCurX()-orbiting.getSize()/2 <= x2 &&
					x1 <= orbiting.absoluteCurX() + orbiting.getSize()/2 &&
					orbiting.absoluteCurY()-orbiting.getSize()/2 <= y2 &&
					y1 <= orbiting.absoluteCurY() + orbiting.getSize()/2)
				{
				...
				if(orbiting instanceof Planet)
				{
					...
					if (cur_planet.getOrbiting() != null) {
						for(Satellite<?> sat : cur_planet.getOrbiting())
						{
							double half_size = sat.getSize()/2;
							if(sat.absoluteCurX() - half_size <= x2 && x1 <= sat.absoluteCurX() + half_size &&
								sat.absoluteCurY() - half_size <= y2 && y1 <= sat.absoluteCurY() + half_size)
							{
		...
		{
			TreeMap<Ship.ShipId, Ship> ships = f.getShips();
			for(Ship.ShipId j : ships.keySet())
			{
				Ship s = ships.get(j);
	...
	private void setDestination(double x, double y, boolean AttackMove)
	{
			...
			for(Satellite<?> sat : orbiting)
			{
				...
				if(sat instanceof Planet)
				{
					...
					if (orbiting2 != null) {
						...
						{
							if (sat2.shouldSelect(x, y, OBJ_TOL))
							{
								dest=sat2;
								break;
							}
		...
		for(Fleet f : sys.getFleets())
		{
			Map<Ship.ShipId, Ship> ships = f.getShips();
			for(Ship.ShipId j : ships.keySet())
			{
				Ship s = ships.get(j);
				
				if(s.shouldSelect(x, y, OBJ_TOL))
				{
					dest = s;
					break;
				}
		...
		for(Selectable the_ship : selected_in_sys)
		{
			...
			if(the_ship != null)
			{
				if(AttackMove)
				{
					...
					{
						GC.scheduleOrder(new ShipAttackOrder(GC.getPlayers()[GC.getPlayer_id()], (Ship)the_ship, TimeControl.getTimeGrainAfter(time), time, (Targetable<?>)dest));
					}	
					else{
						GC.scheduleOrder(new ShipAttackMoveOrder(GC.getPlayers()[GC.getPlayer_id()], (Ship)the_ship, TimeControl.getTimeGrainAfter(time), dest));
					}
					//ShipPanel.updateDestDisplay(dest); //not a bad idea, but its a no-op, since we just only scheduled the order
				}
				else{
					GC.scheduleOrder(new ShipMoveOrder(GC.getPlayers()[GC.getPlayer_id()], (Ship)the_ship, TimeControl.getTimeGrainAfter(time), dest));
					...
					if(dest instanceof Ship && dest != ShipPanel.the_ship&&((Ship)dest).getOwner() != GC.getPlayers()[GC.getPlayer_id()])
					{
						GC.scheduleOrder(new ShipAttackOrder(GC.getPlayers()[GC.getPlayer_id()], (Ship)the_ship, TimeControl.getTimeGrainAfter(time), time, (Targetable<?>)dest));
					}
				}
	...
	{		
		if (!labels_made)
		{
			known_sys=GC.map.getSystems();//GC.players[GC.player_id].known_systems;
	...
	public void mouseWheelMoved(MouseWheelEvent e)
	{
		if(isSystemDisplayed())
		{
			sys_scale *= 1.0-((double)e.getWheelRotation())*Constants.SCROLL_SENSITIVITY;
			if(sys_scale < Constants.MIN_SCALE)
				sys_scale = Constants.MIN_SCALE;
			else if(sys_scale > Constants.MAX_SCALE)
				sys_scale = Constants.MAX_SCALE;
	...
	private void enforceSystemBounds()
	{
		...
		if(sysScreenToDataX(0)<(theinterface.getWidth()-Constants.SYS_WIDTH)/2.0)
			sys_center_x = (int)((theinterface.getWidth()-Constants.SYS_WIDTH)/2.0 + theinterface.getWidth()/(2*sys_scale));//this should be the value of center_x that makes screenToDataX equal to (SystemPanel.getWidth()-SYS_WIDTH)/2
		else if(sysScreenToDataX(theinterface.getWidth())>(theinterface.getWidth()+Constants.SYS_WIDTH)/2.0)
			sys_center_x = (int)((theinterface.getWidth()+Constants.SYS_WIDTH)/2.0 - theinterface.getWidth()/(2*sys_scale));
		//adjust sys_center_y if necessary
		if(sysScreenToDataY(0)<(theinterface.getHeight()-Constants.SYS_HEIGHT)/2.0)
			sys_center_y = (int)((theinterface.getHeight()-Constants.SYS_HEIGHT)/2.0 + theinterface.getHeight()/(2*sys_scale));
		else if(sysScreenToDataY(theinterface.getHeight())>(theinterface.getHeight()+Constants.SYS_HEIGHT)/2.0)
			sys_center_y = (int)((theinterface.getHeight()+Constants.SYS_HEIGHT)/2.0 - theinterface.getHeight()/(2*sys_scale));
	}
	...
	public void updateStats(long time_elapsed, long money, long metal) {
