...
import galactic_strategy.game_objects.Mine;
...
import galactic_strategy.game_objects.TaxOffice;
import galactic_strategy.user_actions.CancelFacilityBuildOrder;
...
{
	private static final long serialVersionUID = 5805586426177285745L;
	
	public Satellite<?> the_sat;
	JButton build;
	JButton cancel;
	JPanel stats_panel;
		JLabel pop;
	...
	public void setSat(Satellite<?> s)
	{
		...
		{			
			...
			if(((OwnableSatellite<?>)s).getOwner() != null)
			{
				...
				if(((OwnableSatellite<?>)s).getOwner().getId() == GameInterface.GC.getPlayer_id())
				{					
	...
	private void setUpFacilityBuilding()
	{
		...
		if(is_building)
		{
			cur_fac_in_prog_panel.add(new JLabel(bldg_in_prog.getIcon()));
	...
	public void update(long t)
	{
		if(the_sat instanceof OwnableSatellite<?>)
		{
			...
			if(((OwnableSatellite<?>) the_sat).getOwner() == GameInterface.GC.getPlayers()[GameInterface.GC.getPlayer_id()])
			{
			...
			no_base_mode=(((OwnableSatellite<?>)the_sat).getThe_base() == null);
		}
	}
	
	public void displayAllFacilities()
	{
		HashMap<Integer, Facility<?>> facilities = ((OwnableSatellite<?>)the_sat).getFacilities();
		synchronized(facilities)
		{
			for(Integer i : new TreeSet<Integer>(facilities.keySet()))
				displayFacility(facilities.get(i));
		}
	}
		...
		if(state == PANEL_STATE.FACILITIES_DISPLAYED)
		{
			...
			switch(f.getType())
			{
				...
				case SHIPYARD:
					...
					Player owner = ((OwnableSatellite<?>)the_sat).getOwner();
					if(owner != null && GameInterface.GC.getPlayer_id() == owner.getId())
	...
	private void facilityChoices()
	{
		...
		for(int i=first_fac; i < end_facs; i++) //start at 1 since NO_BUILDING is type 0
		{
			...
			JLabel ship_pic = new JLabel(fTypes[i].getIcon());
	...
	public void displayQueue()
	{
		state = PANEL_STATE.SHIP_QUEUE_DISPLAYED;
		facilities_panel.removeAll();
		Hashtable<Integer, Ship> manufac_queue = the_shipyard.getManufac_queue();
		synchronized(manufac_queue)
		{
			for(Integer i : manufac_queue.keySet())
			{
				Ship s = manufac_queue.get(i);
				...
				JLabel name_label = new JLabel(s.getType().name());
				...
				JLabel ship_pic = new JLabel(s.getType().getIcon());
	...
	private void displayShipTypes()
	{
		...
		for(int i=1; i < sTypes.length; i++) //start at 1 since MISSILE is type 0
		{
			...
			JLabel name_label = new JLabel(sTypes[i].getName());
			JLabel ship_pic = new JLabel(sTypes[i].getIcon());
