...
import galactic_strategy.XMLEncoder2;
...
public class GDFrame implements Runnable, ActionListener, ChangeListener, MouseMotionListener, MouseListener, WindowListener, KeyListener
{
	...
	int current_nav_level = Constants.DEFAULT_NAV_LEVEL;//saves the nav level from n_slide
	...
	JSpinner t_nav;
	JSlider n_slide;
	JComboBox<String> t_disp_navs;
		int nav_display=Constants.DEFAULT_NAV_OPTIONS; //0=none, 1=selected, 2=all - use next 3 constants
	...
	JCheckBox t_disp_unnav_sys;
		boolean display_unnavigable=Constants.DEFAULT_DISPLAY_UNNAV;
	...
	public void run()
	{
		...
		show_dist= new JSlider(JSlider.HORIZONTAL, 0, Constants.MAX_DIST, Constants.DEFAULT_DIST);
		show_dist.addChangeListener(this);
		show_dist.setMajorTickSpacing(Constants.DIST_MAJOR_TICKS);
		show_dist.setMinorTickSpacing(Constants.DIST_MINOR_TICKS);
		...
		n_slide=new JSlider(JSlider.HORIZONTAL, Constants.MIN_NAV_LEVEL, Constants.MAX_NAV_LEVEL, Constants.DEFAULT_NAV_LEVEL);
		n_slide.addChangeListener(this);
		n_slide.setMinorTickSpacing(Constants.NAV_LEVEL_TICK_SPACING);
		...
		SpinnerModel nav_model=new SpinnerNumberModel(Constants.DEFAULT_NAV_LEVEL, Constants.MIN_NAV_LEVEL, Constants.MAX_NAV_LEVEL, Constants.NAV_LEVEL_TICK_SPACING);//******************************BOOKMARK
		...
		t_disp_navs = new JComboBox<String>(nav_options);
		t_disp_navs.setSelectedIndex(Constants.DEFAULT_NAV_OPTIONS);
	...
	private void save(boolean save_as)
	{
		if(map != null)
		{
			if(map.getName() == null || map.getName() =="")
			{
				String temp_name="";
	...
	private void close()
	{
		if(map != null)
		{
			if(confirmClose())
			{
				map=null;
				cur_file=null;
				panel.paintGalaxy(null,null, DRAG_NONE, Constants.DEFAULT_NAV_LEVEL, Constants.DEFAULT_NAV_OPTIONS, display_unnavigable, null, 1.0d);
	...
	public void nameGalaxy()
	{
		...
		do{
			if(map.getName() == null)
				...
				temp_name = JOptionPane.showInputDialog(frame, "The map's name is: " + map.getName() + "\nType a new name below and click OK, or\n click CANCEL to keep the current name", map.getName());
			if(temp_name == null)
	...
	{		
		...
		for(GSystem sys : map.getSystems()){
			if(sys.getName() != null)
				name_choices.remove(sys.getName());
		}
		...
		for(GSystem sys : map.getSystems()){
			for(Satellite<?> sat : sys.getOrbiting())
			{
				if(sat.getName().length() == 0)
				{
					...
					name_choices.remove(sat.getName());
				}
				...
				if(sat instanceof Planet)
				{
					for(Satellite<?> sat2 : ((Planet)sat).getOrbiting())
					{
						if(sat2.getName().length() == 0)
						{
							...
							name_choices.remove(sat2.getName());
						}
					}
				}
			}
	...
	public void mouseDragged(MouseEvent e)
	{
		if(!wait_to_add_sys && map != null)
		{
			...
			else if(select_but==MouseEvent.BUTTON1)
			{
				...
				ArrayList<GSystem> systems = map.getSystems();
				if(systems != null)
				{
	...
	{
		GSystem new_sys = new GSystem(map.getSystems().size(), x, y, null, new ArrayList<Satellite<?>>(), null, Integer.parseInt(t_nav.getValue().toString()));
	...
	{
		if(enable) {
			map.getSystems().removeAll(selected_systems);
			selected_systems=null;
			drawGalaxy();
