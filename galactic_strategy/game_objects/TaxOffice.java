...
public strictfp class TaxOffice extends Facility<TaxOffice> {
	...
	{
		super(loc, i, Constants.initial_taxoffice_endu);
		location.number_taxoffices++;
		tax_rate = Constants.DEFAULT_INCOME_RATE;
	...
	public double calcTaxingrate(){
		double modified_tax_rate = Constants.DEFAULT_INCOME_RATE;
		for(int j=1;j<location.number_taxoffices;j++){
			modified_tax_rate*=Constants.additional_taxoffice_penalty;
		}
	...
	public void updateStatus(long t) {
		...
		if (t - last_resource_time >= Constants.TIME_BETWEEN_RESOURCES
