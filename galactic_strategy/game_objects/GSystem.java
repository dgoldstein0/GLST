...
public strictfp class GSystem implements Orbitable<GSystem>
{
	...
	public GSystem(int i, int x, int y, String nm, ArrayList<Satellite<?>> orbiting, HashSet<Star> stars, int nav)
	{
		...
		this.y=y;
		navigability=nav;
		fleets = new Fleet[Constants.MAX_PLAYERS];
	...
	{
		//set up ownership
		player_claims = new int[Constants.MAX_PLAYERS];
		for(int i=0; i<Constants.MAX_PLAYERS; i++) //player_claims and fleets both have GalacticStrategyConstants.MAX_PLAYERS as their length
		{
			System.out.println("Setting up fleet " + i + " in system " + name);
			player_claims[i]=0;
			fleets[i] = new Fleet(this, GC.getPlayers()[i]);
		}
	}
		...
		else
		{
			...
			switch(owner_id)
			{
				...
				case OWNER_CONFLICTED:
					...
					return last_color = GameInterface.GC.getPlayers()[current_player_id].getColor();
	
				default:
					return last_color = GameInterface.GC.getPlayers()[owner_id].getColor();
			}
		}
	}
