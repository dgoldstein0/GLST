...
public strictfp class Ship extends Flyer<Ship, ShipId, Fleet.ShipIterator> implements Selectable
{
	...
	public Ship(ShipType t, ShipId id)
	{
		super(t.name,t);
		this.id = id;
	...
	public void userOverride(){
		...
		if(target != null)
		{
			...
			target=null;
			// TODO what if destination is a Ship but not a target?  Is that possible?
		} else {
			AbstractDestination<?> dest = flying_part.getDestination();
			if(dest instanceof Ship)
			{
				((Ship) dest).removeAggressor(this);
			}
		}
	}
	...
	*/
	...
	private void targetLost(long t, LOST_REASON reason, boolean late_order, Targetable<?> tgt /*ignored if late_order is false*/)
	{
		...
		if (flying_part.getDestination() == (AbstractDestination<?>)target &&
				(mode == MODES.ATTACKING || mode == MODES.USERATTACKING))
		{
			...
			double newXCoord = target.getXCoord();
			double newYCoord = target.getYCoord();
			
			// bunch of checks to keep the destination point in bounds
			if (newXCoord > Constants.SYS_WIDTH)
			{
				newXCoord = Constants.SYS_WIDTH;
			}
			else if (newXCoord < 0) {
				newXCoord = 0;
			}
		
			if (newYCoord > Constants.SYS_HEIGHT)
			{
				newYCoord = Constants.SYS_HEIGHT;
			}
			else if (newYCoord < 0) {
				newYCoord = 0;
			}
			flying_part.setDestination(new DestinationPoint(newXCoord, newYCoord));
			flying_part.setCurrent_flying_AI(
					new TrackingAI(flying_part, Constants.LANDING_RANGE, TrackingAI.IN_RANGE_BEHAVIOR.MATCH_SPEED)
				);
		...
		{	
			...
			{
				AbstractDestination<?> destination = flying_part.getDestination();
				double newXCoord = destination.getXCoord();
				double newYCoord = destination.getYCoord();
	...
	class DestUpdater implements Runnable
	{
		final Ship the_ship;
