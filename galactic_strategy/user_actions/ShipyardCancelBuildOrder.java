...
public strictfp class ShipyardCancelBuildOrder extends Order {
	...
	{
		super(t, syd.getOwner());
		
		the_yard=syd;
		shipyard_describer = syd.describer();
		scheduled_time=t;
		ship_id = s.getId().getQueue_id();
		the_ship = s;
	...
	public boolean execute(Galaxy g) throws DataSaverControl.DataNotYetSavedException {
		...
		if(the_yard != null)
		{
			if(the_yard.isAlive() && the_yard.getManufac_queue().size() != 0 &&
					the_yard.getOwner().getId() == p_id) //validity check: is the Shipyard still alive?
			{
				
				if (mode == MODE.NETWORK)
					the_ship = the_yard.getManufac_queue().get(ship_id);
